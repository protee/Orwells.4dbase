...
Function btn_play()
	var $cs_MENU_PLAY : cs:C1710.MENU_PLAY
	var $vT_refMenu; $vT_menu; $vT_UID : Text
	var $cs_SPEECH_PLAY : cs:C1710.SPEECH_PLAY
	var $cE_SPEECHES : cs:C1710.SPEECHESEntity
	
	$cs_MENU_PLAY:=cs:C1710.MENU_PLAY.new()
	$vT_refMenu:=$cs_MENU_PLAY._menu_sessions()
	...
	If ($vT_menu#"")
		Case of 
			...
			: ($vT_menu="SPEECHES-@")
				$vT_menu:=Replace string:C233($vT_menu; "SPEECHES-"; "")
				$vT_UID:=$vT_menu
				$cE_SPEECHES:=ds:C1482.SPEECHES.get($vT_UID)
				$cs_SPEECH_PLAY:=cs:C1710.SPEECH_PLAY.new($cE_SPEECHES)
				
				//QUERY([SPEECHES]; [SPEECHES]ID=$vL_id)
				//F_SPEECH_play_svg
		End case 
	End if 
...
Function btn_menu()
	var $vT_refMenu; $vT_answer; $vT_table : Text
	var $isOk; $is_all : Boolean
	...
	$vC_at_tables.push("SESSIONS"; "SPEECHES")
	$vJ_this:=This:C1470
	$vC_fu_tables:=New collection:C1472()
	$vC_fu_tables.push(Formula:C1597(This:C1470.label); Formula:C1597(This:C1470.label))
	...
	//Function fu_label($cE_CLOCKS : cs.CLOCKSEntity)->$vT_item : Text
	//var $cE_MUSICS : cs.MUSICSEntity
	//var $cE_PERSONS : cs.PERSONSEntity
	//$cE_MUSICS:=$cE_CLOCKS.CLOCKS_MUSICS
	//$cE_PERSONS:=$cE_MUSICS.MUSICS_PERSONS
	//$vT_item:=$cE_CLOCKS.label+" / "+$cE_MUSICS.label
	//$vT_item+=" ↝ "+$cE_PERSONS.firstName+" "+$cE_PERSONS.lastName
